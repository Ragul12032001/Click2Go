...
builder.Services.AddScoped<planexpirychecker>();
builder.Services.AddHostedService<ExpiryChecker>();
...
builder.Services.AddCors(options =>
{
    options.AddPolicy("AllowSpecificOrigin",
        builder => builder.WithOrigins(allowedOrigins.ToArray())
                          ...
                          .AllowAnyMethod()
                          .AllowCredentials());
                          //.SetIsOriginAllowed(origin => true)); // ✅ Allows all origins explicitly
});

//builder.WebHost.UseUrls("http://0.0.0.0:5000"); //Production
...
builder.Services.Configure<PhonePeSettings>(builder.Configuration.GetSection("PhonePe"));
//Configure Serilog


// Base directory for all logs
string baseLogDirectory = @"C:\TravelAD_Api\Logs";
string[] controllers = { "AdvertiserAccount", "Admin", "Authentication", "Whatsapp", "PhonePeWebhook", "Dialler" };
...
builder.Services.AddHostedService<ExpiryChecker>(); // ✅ Campaign Processing
