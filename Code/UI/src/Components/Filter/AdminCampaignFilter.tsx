...
const AdminCampaignFilter: React.FC<DropdownMenuDemoProps> = ({
  visibleColumns,
  setVisibleColumns,
  setColumnsLoading, 
...
console.log("EmailId::" , EmailId);
  ...
  useEffect(() => {
    if (apiUrlAdvAcc) {
      const fetchSavedColumns = async () => {
        setLoading(true);
        try {
          const response = await axios.get(`${apiUrlAdvAcc}/GetAdminCampaignColumns`, {
            params: { EmailId },
          });
  ...
  const saveColumns = async (columns: string[]) => {
    try {
      ...
      const response = await axios.post(`${apiUrlAdvAcc}/UpdateAdminCampaignColumns`, payload, {
  ...
  return (
    <div className="relative w-full">
      ...
      {isDropdownOpen && (
          ...
          <PopoverContent className="w-64 -mt-6 mr-28 bg-white border border-gray-300 rounded shadow-lg z-10 p-4">
            <Command>
              <CommandList>
                <CommandGroup>
            ...
            <div className="mt-2 flex justify-between gap-2">
              <Button variant="ghost" className="text-sm hover:bg-gray-200 border mt-2" onClick={handleClear}>
                Clear
              </Button>
              <Button variant="ghost" className="text-sm hover:bg-gray-200 border mt-2" onClick={() => setIsDropdownOpen(false)}>
                Close
              </Button>
            </div>
          </PopoverContent>
...
export default AdminCampaignFilter;
